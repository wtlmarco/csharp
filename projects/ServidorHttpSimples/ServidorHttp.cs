...
class ServidorHttp
{
    ...
    public SortedList<string,string> DiretoriosHosts { get; set; }
    ...
    private void ProcessarRequest(Socket conexao, int numeroRequest)
    {
        ...
        if(conexao.Connected)
        {
            ...
            if(textoRequisicao.Length > 0)
            {
                ...
                int iL0PrimeiroEspaco = linhas[0].IndexOf(' ');
                int iL0SegundoEspaco = linhas[0].LastIndexOf(' ');
                int iL1PrimeiroEspaco = linhas[1].IndexOf(' ');

                string metodoHttp = linhas[0].Substring(0, iL0PrimeiroEspaco);
                string versaoHttp = linhas[0].Substring(iL0SegundoEspaco + 1);
                string nomeHost = linhas[1].Substring(iL1PrimeiroEspaco + 1);

                //Identifica o arquivo solicitado
                string recursoBuscado = linhas[0].Substring(iL0PrimeiroEspaco + 1, iL0SegundoEspaco - iL0PrimeiroEspaco -1);

                //Redireciona a chamada padrão para o index.html
                if(recursoBuscado == "/") recursoBuscado = "/index.html";
                
                //Extrai qualquer parâmetro passado na URL
                ...
                //Remove qualquer parametro na requisição do nome do recurso
                ...
                FileInfo fiArquivo = new FileInfo(ObterCamihoFisicoArquivo(nomeHost, recursoBuscado));
                if(fiArquivo.Exists)
                {
                    if(TiposMime.ContainsKey(fiArquivo.Extension.ToLower()))
                    {
                        //Identifica o tipo de arquivo com conteúdo dinâmico particular do exemplo
                        //e encaminha para o gerador de conteúdo
                        if(fiArquivo.Extension.ToLower() == ".dhtml")
                            bytesConteudo = GerarHtmlDinamico(fiArquivo.FullName, parametros,metodoHttp);
                        else
                        {
                            //Caso contrário processa o arquivo com os tipos Mimes tradicionais
                ...
                int bytesEnviados = 0;
                bytesEnviados += conexao.Send(bytesCabecalho, bytesCabecalho.Length, 0);
    ...
    {
        StringBuilder hmtl = new StringBuilder();
        hmtl.Append("<!DOCTYPE html><html lang=\"pt-br\"><head><meta charset=\"UTF-8\">");
        hmtl.Append("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">");
        hmtl.Append("<title>Página Estática</title></head><body>");
        hmtl.Append("<h1>Página Estática</h1></body></html>");

        this.HtmlExemplo = hmtl.ToString();
    }
    ...
    {
        string diretorio = "C:\\servers\\apache-php-8.2.4\\htdocs\\csharp\\projects\\ServidorHttpSimples\\www";

        this.DiretoriosHosts = new SortedList<string, string>();
        this.DiretoriosHosts.Add("localhost",diretorio + "\\localhost");
        this.DiretoriosHosts.Add("maroquio.com",diretorio + "\\maroquio.com");

    }

    public string ObterCamihoFisicoArquivo(string host, string arquivo)
    {
