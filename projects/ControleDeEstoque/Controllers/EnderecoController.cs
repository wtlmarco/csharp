...
public class EnderecoController : Controller
{
    ...
    private bool EnderecoExiste(int cid, int eid)
    {
        return _context.Clientes.FirstOrDefault(c => c.IdUsuario == cid)
            .Enderecos.Any(x => x.IdEndereco == eid);
    }
            ...
            if(ModelState.IsValid)
            {
                ...
                if(endereco.IdEndereco > 0)
                {
                    ...
                    if(EnderecoExiste(idUsuario.Value, endereco.IdEndereco))
                    {
                        ...
                        {
                            TempData["mensagem"] = MensagemModel.Serializar("Nenhum dado de endereço foi alterado.", TipoMensagem.Erro);
                        }
                        else
                        {
                            ...
                            {
                                TempData["mensagem"] = MensagemModel.Serializar("Erro ao alterar endereço.", TipoMensagem.Erro);
                            }
                        }
                    }
                    else
