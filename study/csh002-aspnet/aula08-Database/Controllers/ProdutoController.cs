...
public class ProdutoController : Controller
{
    ...
    private SelectList CarregarCategorias()
    {
        ...
        if(ModelState.IsValid)
        {
            if(id.HasValue)
            {
               if(ProdutoExiste(id.Value))
               {
                    ...
                    {
                        TempData["mensagem"] = MensagemModel.Serializar("Produto alterada com sucesso.");
                    }
                    else
                    {
            ...
            else
            {
                ...
                if(await _context.SaveChangesAsync() > 0)
                {
                    TempData["mensagem"] = MensagemModel.Serializar("Produto cadastrada com sucesso.");
                }
                else
                {
    ...
    public async Task<IActionResult> Excluir(int id)
    {
        ...
        if(produto != null)
        {
            ...
            if(await _context.SaveChangesAsync() > 0)
                TempData["mensagem"] = MensagemModel.Serializar("Produto excluída com sucesso.");
            else
                TempData["mensagem"] = MensagemModel.Serializar("Não foi possível excluir a produto.", TipoMensagem.Erro);
        }
        else
        {
            TempData["mensagem"] = MensagemModel.Serializar("Produto não encontrada.", TipoMensagem.Erro);
